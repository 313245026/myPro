...
private:
	...
	bool connectToHttpServer(socketPair& ,const string &);	
	bool sendData(UINT_PTR socket);
	int rcvSocketData(UINT_PTR socket,string &rcvStr);
	bool getHost(const string &rcvData,string &hostName);
	void cleanSocketPair(UINT_PTR );
